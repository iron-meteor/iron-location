...
var lastHistoryWasBack = false;
var setStateFromEventHandler = function () {
  var href = location.href;

  // if the user called Location.back, we know explicitly that the user is going
  // backwards. Alternatively if the click the back button in the browser chrome
  // (or the forward button, etc), we can't tell in what direction they are
  // going, so 'history' is the best we can do.
  var options = {initiator: lastHistoryWasBack ? 'back' : 'history'};
  if (isUsingHashPaths()) {
    var state = new State(urlFromHashStyle(href), options);
    set(state);
  } else {
    var state = new State(href, options);
    set(state);
  }
  lastHistoryWasBack = false;
};

var back = function() {
  lastHistoryWasBack = true;
  history.back();
}
